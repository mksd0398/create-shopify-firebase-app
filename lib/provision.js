...
/** firebase-tools reports an existing project id several different ways. */
function isAlreadyExistsError(message = "") {
  return /already exists|ALREADY_EXISTS|already in use/i.test(message);
}

/**
 * firebase-tools writes ora spinner frames to stderr, so a captured failure is
 * often just "- Creating Google Cloud Platform project". Skip those and return
...
function firstMeaningfulLine(message = "") {
  const lines = message
    ...
    .filter(Boolean)
    .filter((l) => !/^Creating Google Cloud Platform project/i.test(l));
  return lines[0] || "no further detail from firebase-tools";
}

/**
 * Billing (Blaze) status. The Firebase CLI has no command for this, so it
 * needs gcloud. Never throws: an unknown answer is a normal outcome when
 * gcloud is missing, unauthenticated, or lacks billing permission.
 */
function checkBilling(projectId) {
  if (!hasGcloud()) return { known: false, reason: "no-gcloud" };
  ...
  if (typeof out !== "string") {
    ...
    if (/reauthentication|auth login|credentials|not currently have/i.test(msg)) {
      return { known: false, reason: "gcloud-auth" };
    }
    return { known: false, reason: "unknown" };
  }

  const v = out.trim().toLowerCase();
  if (v === "true") return { known: true, enabled: true };
  if (v === "false") return { known: true, enabled: false };
  return { known: false, reason: "unknown" };
}

/** Best-effort browser open - provisioning must never die on this. */
...
/** Used when `firestore:locations` cannot be reached (fresh project). */
const FALLBACK_LOCATIONS = [
  ...
  title: `${value.padEnd(28)} ${c.dim}${city}${c.reset}`,
  value,
}));
...
function fetchLocations() {
  ...
  // `firestore:locations` needs the Firestore API, which a brand-new project
  // does not have enabled yet - so it fails exactly when it is first needed.
  // Returning null here used to skip the region prompt silently and leave the
  // project with no database. Fall back to the common regions instead.
...
export async function provisionFirebase(config, options = {}) {
  ...
  // The caller may have created this project seconds ago. projects:list is
  // eventually consistent, so re-deriving existence here can come back empty
  // and offer to create a project that already exists - which then dies on
  // "already exists". Trust the caller when it says it just created one.
  const projectExists = options.projectJustCreated
    ? true
    : await confirmProjectExists(projectId, login.projects);
  ...
  if (!projectExists && !isCI) {
    ...
    if (shouldCreate) {
      ...
      const out = firebaseRaw(`projects:create ${projectId}`, null, 120000);

      if (typeof out === "string") {
        ok("Firebase project created");
      } else if (isAlreadyExistsError(out.message)) {
        info("Project already exists - adding Firebase resources...");
        firebaseRaw(`projects:addfirebase ${projectId}`, null, 120000);
      } else if (projectExistsDirect(projectId)) {
        // The create reported failure but the project answers project-scoped
        // calls. firebase-tools hides the real reason behind spinner frames,
        // so probe rather than trust the message.
        info("Project is already reachable - continuing");
      } else {
        fail("Could not create project");
        info(firstMeaningfulLine(out.message || ""));
        info(`Manual: ${c.cyan}firebase projects:create ${projectId}${c.reset}`);
        return;
      }
    }
  }
  ...
  // Run this so a later plain `firebase deploy` never stalls on the artifact
  // cleanup-policy prompt. Note it is NOT a billing signal: on a Spark project
  // it reports "repository does not exist" and still exits 0.
  console.log();
  ...
  if (policy.ok) {
    ...
    info(`  firebase functions:artifacts:setpolicy --force --project=${projectId}`);
  }

  // ── 7. Blaze gate ───────────────────────────────────────────
  // Cloud Functions cannot deploy without billing. Pause here so the upgrade
  // happens before the deploy, instead of the deploy dying on it minutes later.
  console.log();
  const upgradeUrl = `https://console.firebase.google.com/project/${projectId}/usage/details`;
  const billing = checkBilling(projectId);

  if (billing.known && billing.enabled) {
    ok("Billing enabled (Blaze) - Cloud Functions can deploy");
    config.billingEnabled = true;
    return;
  }

  if (billing.known && !billing.enabled) {
    warn(`${c.bold}This project is on the Spark (free) plan${c.reset}`);
  } else {
    info(`${c.bold}Cloud Functions require the Blaze (pay-as-you-go) plan${c.reset}`);
  }
  info("The deploy fails without it. Blaze's free tier covers 2M invocations/month.");
  info(`Upgrade: ${c.cyan}${upgradeUrl}${c.reset}`);

  // CI cannot answer a prompt - leave it unknown and let the deploy try.
  if (isCI) {
    config.billingEnabled = null;
    return;
  }

  const { openNow } = await prompts({
    type: "confirm",
    name: "openNow",
    message: "Open the upgrade page in your browser now?",
    initial: true,
  });
  if (openNow) openUrl(upgradeUrl);
  ...
  for (;;) {
    const { next } = await prompts({
      ...
      choices: [
        { title: "Yes - I'm on Blaze, continue the deploy", value: "continue" },
        { title: "Open the upgrade page again", value: "open" },
        { title: "Skip the deploy for now", value: "skip" },
      ],
      initial: 0,
    });
    ...
    // Ctrl+C leaves `next` undefined - treat that as "skip".
    if (next !== "continue") {
      config.billingEnabled = false;
      return;
    }

    // Only contradict the user when it can actually be proven. An answer we
    // cannot verify must never trap them in this loop.
    const recheck = checkBilling(projectId);
    if (recheck.known && !recheck.enabled) {
      warn("Firebase still reports Spark - an upgrade takes a few seconds to land.");
      const { anyway } = await prompts({
        type: "confirm",
        name: "anyway",
    ...
    ok("Continuing - Cloud Functions will deploy");
    config.billingEnabled = true;
    return;
  }
}
